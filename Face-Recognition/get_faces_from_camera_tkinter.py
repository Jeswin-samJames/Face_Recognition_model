...
class Face_Register:
    def __init__(self):
        ...
        tk.Button(self.frame_right_info,
            text='Upload Image',
            command=self.upload_image).grid(row=16, column=0, columnspan=5, sticky=tk.W, padx=5, pady=4)
        

    def upload_image(self):
        file_paths = filedialog.askopenfilenames(filetypes=[("Image files", "*.jpg *.jpeg *.png *.gif *.bmp")])
        if file_paths:
            # Ask the user to input the person's name
            person_name = tkinter.simpledialog.askstring("Input", "Enter person's name:")
            if person_name:
                self.log_all["text"] = f"Images uploaded for {person_name}:"
                self.input_name_char = person_name
                ...
                self.log_all["text"] = "Invalid or empty name. Please try again."
    ...
    def GUI_info(self):
        ...
        tk.Button(self.frame_right_info,
                  ...
                  command=self.save_current_face).grid(row=12, column=1, columnspan=3, sticky=tk.W)
    ...
    def save_current_face(self):
        if self.face_folder_created_flag:
            ...
            self.log_all["text"] = "Please enter the input name"

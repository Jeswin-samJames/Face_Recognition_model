...
conn = sqlite3.connect("facedet.db")
cursor = conn.cursor()
...
class Face_Recognizer:
    def __init__(self):
        ...
        self.recognized_names = set()  # Initialize a set to store recognized names
        self.pronounced_names = set()  # Initialize a set to store names that have been pronounced
    ...
    def details(self, name):
        ...
        cursor.execute("SELECT * FROM details WHERE name = ? AND date = ?", (name, current_date))
        existing_entry = cursor.fetchone()

        if existing_entry:
            print(f"Person has been detected ")
        else:
            current_time = datetime.datetime.now().strftime('%H:%M:%S')
            cursor.execute("INSERT INTO details (name, time, date) VALUES (?, ?, ?)", (name, current_time, current_date))
            conn.commit()
            print(f"Person has been detected for")
                    ...
                    # 6.2.2 / Face cnt increase: 0->1, 0->2, ..., 1->2, ...
                        ...
                        for k in range(len(faces)):
                            ...
                            if min(self.current_frame_face_X_e_distance_list) < 0.4:
                                ...
                                if recognized_name not in self.recognized_names:
                                    nam = recognized_name
                                    ...
                                    if recognized_name not in self.pronounced_names:
                                            # Pass the recognized name as input to another function or page for pronunciation
                                            self.pronounce_name(recognized_name)

                                            # Add the name to the set of pronounced names
                                            self.pronounced_names.add(recognized_name)


                                self.recognized_names.add(recognized_name)
...
#to pronounce the name
    ...
    def pronounce_name(self, name):
        # Use TTS to pronounce the name
        engine = pyttsx3.init()
        engine.say(f"an unauthorized person as entered named {name}.")
        engine.runAndWait()
